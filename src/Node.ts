...
import { Player, Track, UnresolvedTrack } from "./Player.ts";
import {
  connectWebSocket,
...
export class Node {
  ...
  public stats: NodeStats | undefined;
  ...
  constructor(public options: NodeOptions) {
    this.manager = Structure.get("Node")._manager;

    if (!this.manager) throw new RangeError("Manager has not been initiated.");

    if (this.manager.nodes.has(options.identifier || options.host)) {
      this.manager.nodes.get(options.identifier || options.host);
      return;
    }
  ...
  public async connect(): Promise<void> {
    ...
    headers.append("authorization", this.options.password as string);
    headers.append("num-Shards", String(this.manager.options.shards));
    headers.append("user-id", this.manager.options.clientId as string);
    ...
    try {
      ...
      this.manager.emit("nodeError", e);
    }
  }

  protected close({ code, reason }: WebSocketCloseEvent): void {
    this.manager.emit("nodeDisconnect", this, { code, reason });
    if (code !== 1000 || reason !== "destroy") this.reconnect();
  }

  private message(data: Uint8Array | string): void {
  ...
  async open(): Promise<void> {
    ...
    for await (const data of this.socket!) {
      if (isWebSocketCloseEvent(data)) return this.close(data);
  ...
  public send(data: unknown): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      this.socket?.send(JSON.stringify(data)).then(() => resolve(true)).catch(reject);
    });
  }

  private reconnect(): void {
    this.reconnectTimeout = setTimeout(() => {
      if (this.reconnectAttempts >= <number> this.options.retryAmount) {
  ...
  protected trackEnd(
    ...
    if (["LOAD_FAILED", "CLEAN_UP"].includes(payload.reason)) {
      player.queue.previous = player.queue.current as Track;
    ...
    if (track && player.queueRepeat) {
      ...
      if (payload.reason === "STOPPED") {
        ...
        player.queue.add(player.queue.current as Track);
        player.queue.current = player.queue.shift();
      }
