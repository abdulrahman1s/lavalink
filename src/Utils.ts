...
import { Node, NodeStats } from "./Node.ts";
...
export abstract class TrackUtils {
  ...
  static validate(trackOrTracks: unknown): boolean {
    if (
      typeof trackOrTracks === "undefined" || trackOrTracks === null
    ) {
    ...
    if (Array.isArray(trackOrTracks) && trackOrTracks.length) {
      for (const track of trackOrTracks) {
        if (
          !(has.call(track, UNRESOLVED_TRACK_SYMBOL) ||
            has.call(track, TRACK_SYMBOL))
        ) {
          return false;
        }
      }
    ...
    if (typeof trackOrTracks === "object" && trackOrTracks !== null) {
      return (has.call(trackOrTracks, UNRESOLVED_TRACK_SYMBOL) ||
        has.call(trackOrTracks, TRACK_SYMBOL));
    } else {
      return false;
    }
  }
   ...
   * @param track
   */
  static isUnresolvedTrack(track: Track | UnresolvedTrack): boolean {
    if (typeof track === "undefined") {
      throw new RangeError("Provided argument must be present.");
    }
    return has.call(track, UNRESOLVED_TRACK_SYMBOL);
  }
  ...
  static isTrack(track: any): boolean {
    if (typeof track === "undefined") {
      throw new RangeError("Provided argument must be present.");
    }
    return has.call(track, TRACK_SYMBOL);
  }
    ...
    try {
      ...
      if (this.trackPartial) {
        for (const key of Object.keys(track)) {
          ...
          delete track[key as keyof Track];
        }
      }
    ...
    let unresolvedTrack: Partial<UnresolvedTrack> = {
      ...
      async resolve(): Promise<void> {
        ...
        Object.getOwnPropertyNames(this).forEach((prop) =>
          delete this[prop as keyof Partial<UnresolvedQuery>]
        );
  ...
  static async getClosestTrack(
    ...
    const query = [unresolvedTrack.author, unresolvedTrack.title].filter(
      (str) => !!str,
    ).join(" - ");
    ...
    if (unresolvedTrack.duration) {
      const sameDuration = res.tracks.find((track) =>
        (track.duration >= (<number> unresolvedTrack.duration - 1500)) &&
        (track.duration <= (<number> unresolvedTrack.duration + 1500))
      );
