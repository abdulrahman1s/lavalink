...
import { Player, PlayerOptions, Track, UnresolvedTrack } from "./Player.ts";
import {
  LoadType,
  Plugin,
  Structure,
...
export class Manager extends EventEmitter {
  ...
  public async search(
    ...
    const url = `http${node.options.secure ? "s" : ""
      }://${node.options.host}:${node.options.port}/loadtracks`;


    const res: LavalinkResult = await fetch(url + `?identifier=${encodeURIComponent(search)}`, {
      headers: { Authorization: node.options.password as string }
    }).then((d) => d.json());
    ...
    if (result.loadType === "PLAYLIST_LOADED") {
      result.playlist = {
        ...
        selectedTrack: res.playlistInfo.selectedTrack === -1
          ? undefined
          : TrackUtils.build(
            res.tracks[<number>res.playlistInfo.selectedTrack],
            requester,
          ),
  ...
  public async decodeTracks(tracks: string[]): Promise<TrackData[]> {
    ...
    const url = `http${node.options.secure ? "s" : ""
      }://${node.options.host}:${node.options.port}/decodetracks`;

    const res: TrackData[] = await fetch(url, {
      method: "POST",
      headers: { Authorization: node.options.password as string },
    }).then((d) => d.json());
  ...
  public create(options: PlayerOptions): Player {
    return this.players.get(options.guild) ?? new (Structure.get("Player"))(options);
  }
  ...
  public createNode(options: NodeOptions): Node {
    return this.nodes.get(options.identifier || options.host) ?? new (Structure.get("Node"))(options);
  }
   ...
   * @param data
   */
  public updateVoiceState(type: string, data: VoiceState | VoiceServer): void {
    if (
      !data || !["VOICE_SERVER_UPDATE", "VOICE_STATE_UPDATE"].includes(type)
    ) {
      return;
    }
    ...
    if ("session_id" in data && type !== "VOICE_SERVER_UPDATE") {
      if (data.user_id !== this.options.clientId) return;
      state.sessionId = data.session_id;
      if (player.voiceChannel !== data.channel_id) {
        this.emit("playerMove", player, player.voiceChannel, data.channel_id);
        data.channel_id = player.voiceChannel as string;
        player.pause(true);
      }
    } else {
