import { Manager, SearchQuery, SearchResult } from "./Manager.ts";
import { Node } from "./Node.ts";
import { Queue } from "./Queue.ts";
import { Sizes, State, Structure, TrackUtils, VoiceState } from "./Utils.ts";
...
export class Player {
  ...
  constructor(public options: PlayerOptions) {
    ...
    if (this.manager.players.has(options.guild)) {
      this.manager.players.get(options.guild);
      return;
    }
    ...
    const node = this.manager.nodes.get(<string> options.node);
  ...
  public connect(): this {
    ...
    this.manager.options.send(<string> this.guild, {
      op: 4,
      d: {
        guild_id: <string> this.guild,
  ...
  public destroy(): void {
    ...
    this.manager.players.delete(<string> this.guild);
  }
  ...
  public async play(
    ...
    if (
      typeof options.track !== "string" && typeof options.track !== "undefined"
    ) {
  ...
  public seek(position: number): this {
    ...
    if (position < 0 || position > <number> this.queue.current.duration) {
      position = Math.max(
        Math.min(position, <number> this.queue.current.duration),
        0,
      );
    }
